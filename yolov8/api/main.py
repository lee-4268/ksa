...
async def inspection_staging_confirm(request: Request, req: InspStagingConfirmReq):
    ...
    # 기존 좌표 보관: 허가번호 → (위도, 경도)
    # 도로명주소 유무와 무관하게 허가번호 단위로 저장 (설치장소 지오코딩 국소도 포함)
    coord_map: dict = {}
    ...
    # 기존 좌표 복원: 허가번호로만 매칭 (도로명주소 없는 국소도 복원)
    coord_restored = 0
    if coord_map:
...
# ============================================================
# SKO-OCEAN 시설점검 사진 메타 (sisl_photo)
# ============================================================
# 외부 시스템(SKO-OCEAN) 에서 업로드된 시설점검 사진 메타데이터를 보관.
# 실제 이미지는 https://static-int.skons.co.kr/SKO-OCEAN/{FilePath}/{Guid} 에 존재 (사내망 전용).
# 백엔드는 메타만 갖고 URL 을 조립해 응답. 브라우저(사내망) 가 <img> 로 직접 fetch.

_SISL_PHOTO_DB = os.path.join(os.path.dirname(os.path.abspath(__file__)), "sisl_photo.db")
_SISL_PHOTO_BASE_URL = os.environ.get(
    "SISL_PHOTO_BASE_URL", "https://static-int.skons.co.kr/SKO-OCEAN"
)


def _init_sisl_photo_db():
    conn = sqlite3.connect(_SISL_PHOTO_DB, timeout=60)
    conn.execute('PRAGMA journal_mode=WAL')
    conn.execute('''CREATE TABLE IF NOT EXISTS sisl_photo (
        neos_code   TEXT NOT NULL,
        guid        TEXT NOT NULL,
        reg_cls     INTEGER NOT NULL,
        file_path   TEXT NOT NULL,
        upload_date INTEGER NOT NULL,
        PRIMARY KEY (neos_code, guid)
    )''')
    conn.execute('CREATE INDEX IF NOT EXISTS idx_sisl_neos_date ON sisl_photo(neos_code, upload_date DESC)')
    conn.execute('CREATE INDEX IF NOT EXISTS idx_sisl_reg_cls ON sisl_photo(reg_cls)')
    # 임포트 이력 (감사용)
    conn.execute('''CREATE TABLE IF NOT EXISTS sisl_photo_import_log (
        id          INTEGER PRIMARY KEY AUTOINCREMENT,
...
def _build_sisl_photo_url(file_path: str, guid: str) -> str:
    """엑셀의 FilePath ('\\11\\<neos>\\<yyyymm>') + Guid 를 URL 로 조립.

    원본 예시 URL: https://static-int.skons.co.kr/SKO-OCEAN//11/202220942/202605//87C05A48-...
    이중 슬래시가 있는 형태도 동작하므로 그대로 유지.
    """
    # 백슬래시 → 슬래시, 양끝 슬래시 정리
...
@app.post("/admin/sisl-photos/import")
async def admin_sisl_photos_import(request: Request, file: UploadFile = File(...)):
    ...
    def _do_import() -> dict:
        ...
        try:
            # 임포트 전 카운트 (insert vs update 추정용)
            ...
            for ws in wb.worksheets:
                for i, row in enumerate(ws.iter_rows(values_only=True)):
                    if i == 0:
                        # 헤더 검증 (NeOSCode|UploadDate|RegClsCode|Guid|FilePath 순서 가정)
                        continue
