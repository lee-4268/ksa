...
- **V12 결과장 대응 (2026-09-10)**: RAW DATA 신설 2열 파싱 — V열 `부적합`(판정 전용), Y열 `부적합내용`(사유 분류). 기존 U(성능/서류)·W(불합격내용)·X(불합격상세사유)는 유지(X는 상세 서술이라 별도 적재). **부적합 관리 대상 필터는 `부적합='부적합'`(V열) 기준**으로 전환(기존 성능서류 기준은 불합격 유형이라 어긋남 — V 마킹은 작성자가 다음 업로드부터 채움, 초기 건수 적을 수 있음). `/inadequate/sync`는 불합격상세 자리에 Y값만 담음(X는 여러 줄 서술이라 폴백 금지 — 목록·유형집계 깨짐, X 원문은 raw.불합격상세로 조회 가능). sync는 INSERT OR IGNORE 추가형(UNIQUE(year,허가번호))이고 sync-export는 테이블 전체 export라 필터 전환이 기존 관리 이력(심의·완료)을 지우지 않음 — kca 미러(`/inadequate/sync-export`)는 무변경 수신. 실적 미러(`sync-export` `_IRR_MIRROR_COLS`)에 부적합/부적합내용 추가(kca 컬럼명과 1:1 — 다르면 수신부에서 조용히 버려짐). 검사일자는 파싱 시 `_irr_to_ymd`로 `YYYY-MM-DD` 통일(시리얼/datetime/YYYYMMDD/구분자 혼용 정규화, 못 읽는 값은 원문 유지).
