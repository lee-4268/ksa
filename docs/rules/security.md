...
> **2026-08-24 갱신**: 방향 통일 이후 시크릿이 여는 ksa 엔드포인트는 전부 **읽기 전용 export**(sync-export·sync-export-targets·sync-photo-urls·sync-import-file[-data], inspection-results/sync-export, change-request/sync-export, inadequate/sync-export)이며 ksa 데이터 쓰기 경로는 없음. 시크릿 전달 범위: 동기화용 `ksa-sync-config` 는 admin/manager 세션 유지, 특이사항 사진 열람용 `ksa-photo-config`(kca-be) 는 **로그인 사용자 전체** — member 도 수검 준비 시 사진을 봐야 하고, 미러 쓰기(kca-be `/ksa-sync*`)는 세션 role 로 별도 차단되므로 시크릿 보유가 쓰기 권한으로 이어지지 않는다. 유출 시 영향 = ksa 조회 데이터 노출(사용자가 화면에서 이미 보는 범위) + presigned 사진 URL(10분) 발급.
...
| 등급 | 이슈 | 위치 | 조치 | 커밋 |
|---|---|---|---|---|
| High(상) | **4.2 중요정보 노출 제한 — 취약**: `GET /admin/users` 응답에 구성원 email·phone 원문 노출 (관리자>사용자 관리 화면, 진단자 스크린샷 첨부) | routers/users.py:216 | 응답 마스킹 — `_mask_email`(`ab******@sk.com`) 신설 + 기존 `_mask_phone` 적용. 검색(이름/사번/이메일)은 서버측 원본 필터 유지 후 응답만 마스킹 | (이번) |
| 상 | **6.2 중요정보 암호화 저장 — Q&A(미등록)**: 기술 취약점 아님, 통합IT보안진단시스템 설문 미등록 | (시스템 등록) | 등록 답변 근거: 비밀번호 미저장(SSO 위임), DynamoDB encryption at rest, S3 SSE, 시크릿은 systemd env 관리. 시스템에 Q&A 등록 필요 (수동) | - |

> 조치 후 통합IT보안진단시스템 → 취약점조치-인프라에 조치결과 등록 → 정보보호담당 재점검 프로세스. 유사 노출 전수 확인: email/phone 응답 포함 엔드포인트는 `/admin/users`(조치)와 `/users/{empno}`(본인 조회 시에만 — 문제없음) 2곳뿐.
